...
Java_com_ljr_player_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
...
Java_com_ljr_player_Player_native_1start(JNIEnv *env, jobject instance, jstring path_,
                                         jobject surface) {
    ...
    av_image_alloc(dst_data, dst_linesize, codecContext->width, codecContext->height,
                   AV_PIX_FMT_RGBA, 1);
    AVFrame *frame = av_frame_alloc();//接收的容器
